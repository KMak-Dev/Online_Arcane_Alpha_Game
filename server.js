...
const Game = (function() {
    ...
    function startGame() {
        ...
        for(let i = 494; i < 506; i++) {
            map.setTile(i, 219, 4);     // Upper Tile
            map.setTile(i, 225, 20);    // Bottom Tile
            for(let j = 220; j < 225; j++) {
                map.setTile(i, j, 12);  // Center Tile
            }
        }
        for(let j = 220; j < 225; j++) {
            map.setTile(493, j, 11);    // Left Tile
            map.setTile(506, j, 13);    // Right Tile
        }
        map.setTile(493, 219, 3);       // Upper Left Tile
        map.setTile(506, 219, 5);       // Upper Right Tile
        map.setTile(493, 225, 19);      // Lower Left Tile
        map.setTile(506, 225, 21);      // Lower Right Tile
        ...
        for(let i = 0; i < 8; i++) {
            for(let j = 0; j < 3; j++) {
                interactableStates[currentInteractableID] = { ...primaryPedestals[Math.floor(Math.random() * primaryPedestals.length)] };
            ...
            interactableStates[currentInteractableID] = { ...secondaryPedestals[Math.floor(Math.random() * secondaryPedestals.length)] };
            interactableStates[currentInteractableID++].pos = { x: possibleSpawn[i].x - 100 * Math.sign(possibleSpawn[i].x) + 20, y: possibleSpawn[i].y - 100 * Math.sign(possibleSpawn[i].y) + 20 }
        }
        ...
        for(i in teams) {
            for(let j = teams[i].length; j < 3; j++) {
                ...
                aiCore["Bot " + botNumber] = {
                    ...
                    shroudAversion: 1,     // Priority of moving towards center
                }
    ...
    // Update loop
    function update() {
        for(player in playerStates) updatePlayerState(player);
    ...
    const spells = [
        { empty: true },                                                                                                // Empty
        { velocity: 500, life: 1, damage: 60, spread: 10, knockback: 150, recoil: 100, cooldown: 0.2,  cost: 10 },       // Magic Missile
        { velocity: 300, life: 1, damage: 69420, spread:  0, knockback: 150, recoil:  10, cooldown: 0.05, cost: 5 },       // Killing Curse
        { velocity: 200, life: 1, damage: 30, spread: 20, knockback:  50, recoil:  50, cooldown: 0.1,  cost: 1  },       // Fire Bolt
        { empty: true },                                                                                                // Empty
        { empty: true },                                                                                                // Empty
        { empty: true },                                                                                                // Empty
        { empty: true },                                                                                                // Empty
        ...
        { empty: true },                                                                                                // Empty
        { velocity: 10, life: 3, damage:  0, spread: 0, knockback:   0, recoil:  20, cooldown: 2, cost: 50 },           // Shield
    ];
    ...
    function tryInteract(player) {
        ...
        if(type >= 0 && type <= 4) {
            ...
            playerStates[player].socket.emit("set spell", type - 6);
    ...
    function isInShroud(x, y) {
        ...
        else if(time <= 90) { // Full to Half
            cornerX = mapWidth  / 4 + ( mapWidth / 4 ) * (90 - time) / 30;
            cornerY = mapHeight / 4 + ( mapWidth / 4 ) * (90 - time) / 30;
        } else if(time <= 150) {
            cornerX = mapWidth  / 4;
            cornerY = mapHeight / 4;
        } else if(time <= 180) { // Half to arena
            cornerX = 160 + ( mapWidth  / 4 - 160 ) * (180 - time) / 30;
            cornerY =  90 + ( mapHeight / 4 -  90 ) * (180 - time) / 30;
        } else if(time <= 210) {
            cornerX = 160;
            cornerY = 90;
        } else if(time <= 240) {
            cornerX = 160 * (240 - time) / 30;
            cornerY =  90 * (240 - time) / 30;
        }
    ...
    function updateLootState(id) {
        ...
        for(player in playerStates) {
            ...
            if(_x * _x + _y * _y <= 8 * 8) {
                playerStates[player].exp += 5;
                if(playerStates[player].exp >= playerStates[player].level * 2) {

                    playerStates[player].exp -= playerStates[player].level * 2;
                    playerStates[player].level += 1;

                    playerStates[player].maxHealth += 20;
                    playerStates[player].health += 20;
    ...
    function botAction(player) {
        ...
        scores[playerStates[player].movement] += aiCore[player].inertia;
        for(i in scores) {
            let dirX = (i & 2) / 2 - (i & 8) / 8;
            let dirY = (i & 4) / 4 - (i & 1) / 1;
            
            const dot = dirX * playerStates[player].pos.x + dirY * playerStates[player].pos.y;
            scores[i] -= Math.sign(dot) * aiCore[player].shroudAversion;
        }
    ...
    function KillPlayer(player, killer) {
        ...
        // Spawn loots
        for(let i = 0; i < playerStates[player].level + 2; i++) {
